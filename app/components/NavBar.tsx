    ...
    >
      <div className="mx-auto flex min-w-0 max-w-6xl items-center justify-between gap-2 px-4 py-3 sm:px-6 lg:py-4">
        ...
        >
          <span
            aria-hidden
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-green-500 to-emerald-600 text-sm font-bold text-white shadow-sm"
          >
            FM
          </span>
        ...
        <div className="hidden items-center gap-3 lg:flex lg:gap-3 xl:gap-6">
          ...
          <Link
            href={contactHref}
            className="whitespace-nowrap rounded-lg bg-green-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-green-500"
          >
            {t.cta}
          </Link>
        </div>
      ...
      {menuOpen && (
        <div className="max-h-[min(70vh,calc(100dvh-env(safe-area-inset-top)-5rem))] space-y-3 overflow-y-auto border-t border-zinc-200/80 bg-white/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md dark:border-zinc-800/60 dark:bg-zinc-950/95 lg:hidden">
          ...
          <Link
            href={contactHref}
            onClick={() => setMenuOpen(false)}
            className="block rounded-lg bg-green-600 px-4 py-3 text-center text-sm font-medium text-white transition hover:bg-green-500"
          >
            {t.cta}
          </Link>
        </div>
